...
function dijk(){
  ...
  while(queue.length > 0){
    ...
    if(currentNode.location[0] === input.length - 1 && currentNode.location[1] === input[1].length - 1){
    ...
    queue = queue.sort((a,b) => a.heat - b.heat);
  }
}
...
function move(node, direction) {
  ...
  newNode.heat = node.heat + parseInt(input[newNode.location[0]][newNode.location[1]]);
